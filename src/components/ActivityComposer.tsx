...
export function ActivityComposer({ userId, onClose, onSaved }: { userId:string; onClose:()=>void; onSaved:()=>void }) {
  ...
  return <div className="panel-backdrop"><aside className="profile-panel"><div className="panel-header"><h2>Create activity</h2><button className="icon-button" onClick={onClose}><X/></button></div><form className="profile-form" onSubmit={submit}><label>Title<input name="title" required maxLength={80}/></label><label>Description<textarea name="description" rows={3}/></label><label>Category<select name="category"><option>Active</option><option>Chill</option><option>Food</option><option>Creative</option></select></label><label>For a team (optional)<select name="team"><option value="">Everyone</option>{teams.map(t=><option key={t.id} value={t.id}>{t.kind==='house'?'House':'Design'} — {t.name}</option>)}</select></label><label>Location<select name="location" required><option>Volley Ball Field</option><option>Cafeteria</option><option>Floor 2 Lounge</option><option>Floor 3 Lounge</option><option>Floor 4 Lounge</option><option>Floor 5 Lounge</option><option>Roll Call</option><option>Lobby</option><option>Fireside</option></select></label><div className="form-row"><label>Starts<input type="datetime-local" name="starts" required/></label><label>Ends<input type="datetime-local" name="ends" required/></label></div><label>Capacity<input type="number" name="capacity" min="2" max="100" defaultValue="8" required/></label><button className="auth-submit">Publish activity</button>{message&&<p className="form-message">{message}</p>}</form></aside></div>
}
